...
1. Распаковать внешний архив TiareTravel_GitHub_v0.4.0.zip в приложении «Файлы».
...
Загрузить весь комплект в корень существующего репозитория 7992811/TiareTravel с заменой совпадающих файлов. Новый проект и новую базу создавать не нужно. Render автоматически выполнит обновление после Commit changes. Дождаться статуса Live и обновить страницу кабинета.

## PDF-предложения

В кабинете агентства открыть заявку и выбрать вариант. «PDF для проверки» открывает документ по сохранённой версии выбранного варианта, даже до публикации. В документе явно отмечаются условные бюджеты и неподтверждённые детали. Новый текст в поле «Комментарий клиенту» нужно предварительно записать кнопкой «Сохранить комментарий»: он хранится у выбранного варианта и не публикуется автоматически.

После публикации маршрута появляется «PDF для клиента»: он использует опубликованный снимок, без внутренних заметок, других кандидатов и демонстрационных сумм. Действующая подтверждённая стоимость берётся из проверки цены; если её ещё нет или срок истёк, PDF сообщает, что стоимость требует подтверждения.

PDF открывается в новой вкладке. На iPhone использовать «Поделиться» → «Сохранить в Файлы» либо выбрать почту/мессенджер. Генерация PDF не публикует маршрут и не отправляет письмо. Изменения варианта и комментария нужно сохранить до выгрузки; изменение опубликованного маршрута требует повторной публикации. Файл фиксирует номер заявки, ревизию и дату формирования.

Карты используют локальную географическую основу Natural Earth. Точки показывают географические ориентиры, линии между ними — последовательность, а не подтверждённую траекторию. Неизвестные места, транспорт и время отмечаются как требующие уточнения. Внешние карты, браузерный движок и платные API для создания PDF не требуются.

## Первый запуск в Render
...
- tiare-pdf-*.ttf, tiare-pdf-cover.jpg и pdf-geography.json — локальные ресурсы PDF. Шрифты и географический справочник не выдаются как публичные файлы сайта.
